...
ACTION:FILL_FORM:{"custName":"M/S Sharma Rice Mills","custContact":"Rajesh Sharma","custPhone":"9876543210","custEmail":"sharma@gmail.com","custLoc":"Punjab","plantType":"Rice Mill","capacity":"6","turnkey":true}"`;
  ...
  try {
    ...
    if (!response.ok) {
      let friendlyMsg = 'AI service se connection nahi ho paya. Thodi der baad try karein.';
      try {
        const errData = await response.json();
        const errType = errData?.error?.type || '';
        if (errType === 'invalid_request_error' && errData?.error?.message?.includes('credit')) {
          friendlyMsg = 'AI credits khatam ho gayi hain. console.anthropic.com → Billing mein credits add karein.';
        } else if (response.status === 401) {
          friendlyMsg = 'API key galat hai. Vercel Environment Variables mein sahi key daalen.';
        } else if (response.status === 429) {
          friendlyMsg = 'Bahut zyada requests ho gayi. Thodi der ruk ke dobara try karein.';
        }
      } catch(_) {}
      return res.status(200).json({
        content: [{ type: 'text', text: '⚠️ ' + friendlyMsg }]
      });
    }
    ...
    return res.status(200).json({
      content: [{ type: 'text', text: '⚠️ Server se connect nahi ho paya: ' + err.message }]
    });
  }
};
